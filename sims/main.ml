...
let make_setup ~systematic ~data_block_count ~drop_count ~data_block_size ~data_loss_rate ~rounds =
...
let run (setup : setup) : stats =
  let rec aux (original_data_blocks : Cstruct.t array) (decode_ctx : Ofountain.decode_ctx) (stats : stats) (drops : Ofountain.drop Seq.t) =
    match drops () with
    | Seq.Nil -> stats
    | Seq.Cons (x, xs) ->
      ...
      | Ok (`Success arr) -> (
        for i=0 to Ofountain.Param.data_block_count (Ofountain.param_of_decode_ctx decode_ctx) -1 do
  ...
  in
  let data_blocks =
    Array.init (Ofountain.Param.data_block_count setup.param)
    (fun _ -> Cstruct.create setup.data_block_size)
  in
  ...
  let drop_data_buffer =
    Array.init (Ofountain.Param.drop_count setup.param) (fun _ -> Cstruct.create setup.data_block_size)
  in
  let drops =
    (
      Result.get_ok @@
      Ofountain.encode_with_param_lazy
      ~drop_data_buffer
      setup.param
      data_blocks
    )
  in
  let decode_ctx = Result.get_ok @@ Ofountain.make_decode_ctx ~data_block_buffer
    ~data_block_size:setup.data_block_size
    setup.param
  in

module Param : sig
  ...
  val make : systematic:bool -> data_block_count:int -> drop_count:int -> layer_step_ratio:float -> (t, error) result
...
let make ~systematic ~data_block_count ~drop_count ~layer_step_ratio =
  ...
  if drop_count <= 0 || data_block_count > Constants.max_data_block_count then
    Error `Invalid_drop_count
  else
  if layer_step_ratio <= 0.1 || layer_step_ratio > 1.0 then
  Error `Invalid_layer_step_ratio
  else
    Ok { systematic; data_block_count; drop_count; layer_step_ratio }
...
let calc_layer_count param =
  ...
  int_of_float @@
  Float.ceil (float_of_int drop_count /. step_size)
...
module Encode = struct
  ...
  let create_encoder ~data_blocks ~drop_data_buffer param : (encoder, error) result =
    if Array.length data_blocks <> Param.data_block_count param then
      ...
      | Ok drop_data_buffer -> (
                ...
                Array.init layer_count (fun i ->
                  if i = 0 then
                    ...
                    Array.init layer_size (fun i ->
                      if i < Array.length !prev_layer then
                        !prev_layer.(i)
                      else
          ...
          let lt_encoders =
            ...
            in
            Array.init layer_count (fun i ->
              Lt_code.create_encoder
              ~data_blocks:data_block_layers.(i)

...
let xor_onto ~(src : Cstruct.t) ~(onto : Cstruct.t) : unit =
  ...
  assert (Cstruct.length src = Cstruct.length onto);
  let len = Cstruct.length src in
  let src = coerce (ptr char) (ptr uchar) (bigarray_start array1 (Cstruct.to_bigarray src)) in
  let onto = coerce (ptr char) (ptr uchar) (bigarray_start array1 (Cstruct.to_bigarray onto)) in
  Xor_c.xor_onto len src onto
